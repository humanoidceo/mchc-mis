...
def add_pharmacist_family_planning_permission(apps, schema_editor):
    StaffProfile = apps.get_model('accounts', 'StaffProfile')
    for profile in StaffProfile.objects.filter(role='pharmacist').iterator():
...
def remove_pharmacist_family_planning_permission(apps, schema_editor):
    StaffProfile = apps.get_model('accounts', 'StaffProfile')
    for profile in StaffProfile.objects.filter(role='pharmacist').iterator():
...
class Migration(migrations.Migration):
    dependencies = [
        ('accounts', '0009_private_documents_permission'),
    ]

    operations = [
        migrations.RunPython(add_pharmacist_family_planning_permission, remove_pharmacist_family_planning_permission),
    ]
