...
import { ChevronLeft, ChevronRight, LayoutDashboard, Syringe, Trash2, User } from 'lucide-react'
...
const links = [
  { to: '/vaccination/dashboard', label: 'Dashboard', icon: LayoutDashboard },
  { to: '/vaccination/records', label: 'Vaccinations', icon: Syringe },
  { to: '/vaccination/account', label: 'My account', icon: User },
]
      ...
      <div className={`transition-all duration-300 ${sidebarCollapsed ? 'lg:pl-20' : 'lg:pl-64'}`}>
        <header className="no-print sticky top-0 z-10 border-b border-sky-100 bg-white/95 px-4 py-3 backdrop-blur">
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-3">
              <NavLink to="/vaccination/trash" className="rounded border border-sky-200 bg-white px-3 py-2 text-sm font-medium hover:bg-sky-50">
        ...
        <main className="mx-auto max-w-7xl px-4 py-6">
          <Routes>
            ...
            <Route path="/vaccination/trash" element={<TrashBinPage />} />
